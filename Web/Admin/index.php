...
function   Serial()
	{
		...
		$fp = fopen("/home/pi/.ID/Datos_Creacion.txt", "r");
		$linea = fgets($fp);
		$dato .= $linea;
		$linea = fgets($fp);
		$dato .= " -XXXXX- ";
...
function   Version_Firmware()
	{
		...
		$fp = fopen("/home/pi/Firmware/README.md", "r");
		$linea = fgets($fp);
		$dato .= $linea;
		fclose($fp);
		...
		$fp = fopen("/home/pi/Firmware/db/Config/Vercion_Firmware.txt", "r");
		$linea = fgets($fp);
		$dato .= $linea;
		fclose($fp);
	...
	function   Procesos()
		{
			...
			$fp = fopen("/home/pi/Firmware/auto/Procesos.txt", "r");
					//$linea = fgets($fp);
					//$dato .= $linea;
					while(!feof($fp)) {
						$linea = fgets($fp);
